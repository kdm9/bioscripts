...
CLI_OPTS = """
USAGE:
grid2list.py -i <96wellPlate> -d <dict> [-o <table> -s <sep>]

OPTIONS:
    -i <96wellPlate>    A CSV containing a 96 well plate layout
    -d <dictfile>       An experiment as a json
...
def print_list(cells, outfn, sep, dct):
    lines = []
    cols = ["Index","Cell", "Value"]
    cols.extend(dct[dct.keys()[0]].keys())
    lines.append(sep.join(cols)) # add header
    for iii, cell in enumerate(cell_names):
        line_values = [str(iii + 1), cell, cells[cell]]
        try:
            line_values.extend(dct[cells[cell]].values())
        except KeyError:
            print cells[cell]
            line_values.extend([""] * len(dct[dct.keys()[0]].keys()))
        lines.append(sep.join(line_values))
    if outfn != "stdout":
...
if __name__ == "__main__":
    ...
    with open(opts['-d']) as fh:
       dct = json.load(fh)
    grid = parse_grid(opts['-i'])
    print_list(grid, opts['-o'], opts['-s'], dct)
